...
def expiry_report():
    ...
    return render_template(
        "reports/expiry.html", products=products, days_filter=days_filter
    )
...
def outstanding_payments():
    ...
    customers = Customer.query.filter(
        Customer.company_id == company_id, Customer.current_balance > 0
    ).all()

    suppliers = (
        db.session.query(Supplier, func.sum(Purchase.total_amount).label("outstanding"))
        .outerjoin(Purchase, Purchase.supplier_id == Supplier.id)
        .filter(Supplier.company_id == company_id, Purchase.payment_status != "paid")
        .group_by(Supplier.id)
        .all()
    )

    return render_template(
        "reports/outstanding_payments.html", customers=customers, suppliers=suppliers
    )
