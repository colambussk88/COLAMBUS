...
def dashboard():
    ...
    today_returns = (
        db.session.query(func.sum(SalesReturn.refund_amount))
        .join(Sale, SalesReturn.sale_id == Sale.id)
        .filter(
            Sale.company_id == company_id, func.date(SalesReturn.return_date) == today
        )
        .scalar()
        or 0
    )
    ...
    total_profit = (
        ...
        .join(Product, SaleItem.product_id == Product.id)
        .join(Sale, SaleItem.sale_id == Sale.id)
        .filter(Sale.company_id == company_id, Sale.is_cancelled == False)
        .scalar()
        or 0
    )
    ...
    total_purchase_returns = (
        db.session.query(func.sum(PurchaseReturn.credit_amount))
    ...
    # -------------------------------
    recent_sales = (
        Sale.query.filter(Sale.company_id == company_id, Sale.is_cancelled == False)
        .order_by(Sale.invoice_date.desc())
        .limit(10)
        .all()
    )
    ...
    for i in range(7):
        ...
        returns = (
            ...
            .join(Sale, SalesReturn.sale_id == Sale.id)
            .filter(
                Sale.company_id == company_id, func.date(SalesReturn.return_date) == day
            )
            .scalar()
            or 0
        )
    ...
    # -------------------------------
    total_stock_value = (
        db.session.query(func.sum(Product.quantity * Product.purchase_price))
