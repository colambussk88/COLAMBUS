...
def pos():
    ...
    doctors = (
        Doctor.query.filter_by(company_id=current_user.company_id)
...
def checkout():
    ...
    try:
        ...
        for item in items:
            ...
            # ✅ SAFELY GET TAX
            tax_percent = float(product.tax_percentage or 0)

               # ✅ CALCULATE SUBTOTAL
            subtotal_item = (unit_price * quantity) - item_discount

                 # ✅ CALCULATE TAX
            tax_amount = 0
            if include_tax:
             tax_amount = subtotal_item * (tax_percent / 100)

            # ✅ FINAL ITEM TOTAL
            item_total = subtotal_item + tax_amount
...
def invoice_detail(sale_id):
    ...
    return render_template("sales/invoice_detail.html", sale=sale, returns=returns)
...
def download_invoice_pdf(sale_id):
    ...
    try:
        ...
        elements.append(
            Paragraph(
                company_info, ParagraphStyle("company", fontSize=9, fontName="DejaVu")
            )
        )
        ...
        bill_table = Table(
            [
                [
                    Paragraph(
                        f"<b>Bill To</b><br/>{sale.customer_name or 'Walk-in Customer'}",
                        ParagraphStyle("normal", fontName="DejaVu"),
                    ),
                    Paragraph(
                        f"<b>Sold By</b><br/>{company.company_name}",
                        ParagraphStyle("normal", fontName="DejaVu"),
                    ),
                ]
            ],
    ...
    if request.method == "POST":
        try:
            ...
            if is_full_return:
                ...
                return_credit = (sale_item.unit_price * quantity) + (
                    sale_item.tax_amount * quantity / sale_item.quantity
                )
